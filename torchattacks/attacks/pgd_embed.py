...
class PGD(Attack):
    ...
    def forward(self, images, labels):
        ...
        min = torch.min(image)-200
        max = torch.max(image)+100
        ...
        if universal == 1:
            ...
            noise = torch.zeros(1, 64, 4096).to(self.device)
            #print(noise)
        ...
        for _ in range(self.steps):
            ...
            for k in range(len(images_)):
                ...
                for p in range(self.nprompt):
                    ...
                    adv_image = torch.clamp(image + delta, min=min, max=max).detach()
        ...
        if universal == 1: #universal noise
            ...
            for k in range(len(images_)):
                ...
                adv_image = torch.clamp(images_[k] + delta, min=min, max=max).detach()
