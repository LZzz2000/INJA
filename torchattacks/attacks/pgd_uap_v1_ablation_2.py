...
from ..attack_ablation_2 import Attack
...
class PGD(Attack):
    ...
    def forward(self, images, labels):
        ...
        if universal == 1:
            noise = torch.zeros(1, 256, 4096).to(self.device)
            #print(noise)
        ...
        for i in range(self.steps):
            ...
            if (i+1) % 50 == 0:
                ablation_img.append(torch.clamp(images_[0] + torch.clamp(noise, min=-self.eps, max=self.eps), min=0, max=1).detach())
            ablation_loss.append(cost_step)
        ...
        if universal == 1: #universal noise
            ...
            return images_outputs_, ablation_img, ablation_loss
