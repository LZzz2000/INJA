...
# def get_dynamic_alpha(step, total_steps, min_alpha=1/255, max_alpha=16/255):
#     """
#     Gradually increases alpha from min_alpha to max_alpha over total_steps.
#     """
#     alpha = min_alpha + (max_alpha - min_alpha) * (step / total_steps)
#     return alpha
...
class PGD(Attack):
    ...
    def __init__(self, model, eps=8 / 255, alpha=2 / 255, steps=10, nprompt=1, random_start=True, universal=1, last_n_steps=10, noise_size = 224):
    ...
    def forward(self, images, labels = None):
        ...
        if self.universal == 1:
            noise = torch.zeros(1, 3, self.noise_size, self.noise_size).to(self.device)
        ...
        for _ in range(self.steps):
            ...
            for k in range(len(images_)):
                ...
                if _ >= self.steps - self.last_n_steps:
                    history.append(noise.clone().detach())
        ...
        if self.universal == 1:  # universal noise
            ...
            last_10_adv_images = []
            for noise_hist in history:
                adv_img = images_[0] + noise_hist
                for c in range(3):
                    adv_img[:, c:c + 1] = torch.clamp(
                        adv_img[:, c:c + 1],
                        min=self.min_values[0, c],
                        max=self.max_values[0, c]
                    )
                last_10_adv_images.append(adv_img.detach())

            return images_outputs_,  last_10_adv_images